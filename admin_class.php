...
Class Action {
	...
	function save_country(){
		...
		if(empty($id)){
			...
			$save = $this->db->query("INSERT INTO country set $data");
		}else{
			$save = $this->db->query("UPDATE country set $data where country_id  = $id");
		}
		if($save){
			return 1;
		}
	...
	function save_branch(){
		...
		if(empty($id)){
			...
			while($i == 0){
				...
				$chk = $this->db->query("SELECT * FROM prices where prices_code = '$bcode'")->num_rows;
				if($chk <= 0){
					$i = 1;
				}
			}
			$data .= ", prices_code='$bcode' ";
			$save = $this->db->query("INSERT INTO prices set $data");
		}else{
			$save = $this->db->query("UPDATE prices set $data where id = $id");
		}
		if($save){
			return 1;
		}
	...
	function delete_branch(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM prices where id = $id");
		if($delete){
			return 1;
		}
	}
	function delete_country(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM country where country_id = $id");
		if($delete){
			return 1;
		}
	}
